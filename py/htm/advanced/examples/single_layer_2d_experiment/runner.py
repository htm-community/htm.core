...
from htm.advanced.algorithms.single_layer_location_memory import SingleLayerLocationMemory
...
class SingleLayerLocation2DExperiment(object):
    ...
    def doTimestep(
        ...
        for monitor in list(self.monitors.values()):
            monitor.beforeTimestep(locationSDR, transitionSDR, featureSDR, egocentricLocation, learn)
    ...
    def learnObjects(self, objectPlacements):
        ...
        for objectName, objectDict in self.objects.items():
            ...
            for locationName, featureName in objectDict.items():
                ...
                self.inputRepresentations[(featureName, egocentricLocation)] = (self.inputLayer.getActiveCells())
    ...
    def _selectTransition(self, allocentricLocation, objectDict, visitCounts):
        ...
        for transition in candidates:
            ...
            if (selectedVisitCount is None or visitCounts[candidateLocation] < selectedVisitCount):
    ...
    def inferObjectsWithRandomMovements(self, objectPlacements, maxTouches=20, settlingTime=2):
        """
        ...
        for objectName, objectDict in self.objects.items():
            ...
            for _ in range(maxTouches):
                ...
                if (set(self.objectLayer.getActiveCells()) == set(self.objectRepresentations[objectName])
                    and set(self.inputLayer.getActiveCells()) == set(self.inputRepresentations[(featureName, learnedEgocentricLocation)])
                    and set(self.locationLayer.getActiveCells()) == set(self.locations[learnedEgocentricLocation])
                    ):
                    break
                else:
                    allocentricLocation, transitionName = self._selectTransition(allocentricLocation, objectDict, visitCounts)
                    nextTransitionSDR = self.transitions[transitionName]
